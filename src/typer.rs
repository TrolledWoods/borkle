...
fn build_constraints(ctx: &mut Context<'_, '_>, node_id: NodeId) -> type_infer::ValueId {
    ...
    match node.kind {
        ...
        NodeKind::FunctionDeclaration { ref args, returns, body } => {
            ...
            for (local_id, type_node) in args {
                ...
                ctx.infer.set_equal(type_id, local_type_id, Variance::Invariant);
                function_type_ids.push(type_id);
            }

            let body_type_id = build_constraints(ctx, body);
            ctx.infer.set_equal(body_type_id, returns_type_id, Variance::Variant);

            let infer_type = type_infer::ValueKind::Type(
                Some((type_infer::TypeKind::Function, Some(function_type_ids.into_boxed_slice())))
            );
            let infer_type_id = ctx.infer.add(infer_type);
            ctx.infer.set_equal(infer_type_id, node_type_id, Variance::Invariant);
        }
        NodeKind::FunctionCall { calling, ref args } => {
            ...
            ctx.infer.set_field_equal(calling_type_id, 0, node_type_id, Variance::Invariant);

            for (i, &arg) in args.iter().enumerate() {
                let arg_type_id = build_constraints(ctx, arg);
                ctx.infer.set_field_equal(calling_type_id, i + 1, arg_type_id, Variance::Covariant);
            }

            // Specify that the caller has to be a function type
            let infer_type = type_infer::ValueKind::Type(Some((type_infer::TypeKind::Function, None)));
            let type_id = ctx.infer.add(infer_type);
            ctx.infer.set_equal(calling_type_id, type_id, Variance::Invariant);
        }
